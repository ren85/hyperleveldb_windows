...
#pragma GCC diagnostic pop
	...
	bool SomeFileOverlapsRange(
		...
		if (smallest_user_key != NULL) {
			...
			InternalKey small_(*smallest_user_key, kMaxSequenceNumber, kValueTypeForSeek);
			index = FindFile(icmp, files, small_.Encode());
		}
	...
	void Compaction::AddInputDeletions(VersionEdit* ed) {
		for (int which = 0; which < 2; which++) {
			for (size_t i = 0; i < inputs_[which].size(); i++) {
				ed->DeleteFileVer(level_ + which, inputs_[which][i]->number);
			}
		}
	}
